 ...
 * @property {Number} connectionAttempts the number of attempts to do when trying to connect to a server
 * @property {Proxy.ProxyConfiguration} proxyConfiguration the proxy configuration to connect to a server
 * @constructor
 */
function Configuration () {
    this.defaultConsistency   = Types.Consistency.ABSOLUTE;
    this.defaultDurability    = new Types.Durability(Types.SyncPolicy.NO_SYNC, Types.ReplicaAckPolicy.ALL, Types.SyncPolicy.NO_SYNC);
    this.connectionAttempts   = 3;
    this.iteratorBufferSize   = 100;
    this.proxy                = new Proxy.ProxyConfiguration();
}
